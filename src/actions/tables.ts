...
export async function updateTable(
  ...
  const payload: Record<`${Wind}_id`, string | null> = {
    east_id: players.east?.id ?? null,
    south_id: players.south?.id ?? null,
    west_id: players.west?.id ?? null,
    north_id: players.north?.id ?? null,
  };
